...
namespace SZUtilities
{
    ...
    {
        private new static DebugEx Debug = new DebugEx(nameof(PostprocessObject));
        ...
        {
            var objectsToPostprocess = Resources.FindObjectsOfTypeAll<PostprocessObject>()
                .Where(x => !UnityEditor.EditorUtility.IsPersistent(x)).ToArray();
        ...
        protected override void Awake()
        {
            ...
            Debug.LogWarning(
                $"PostProcess {m_action} {gameObject.scene.name}:{gameObject.name}: executed in game mode!");
